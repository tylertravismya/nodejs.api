	...
	generateAppHelper: (appParams, techStackId, modelId, appOptionsPath, gitParams) => {
		return new Promise(async function(resolve, reject) {
			...
			if ( appOptionsFileToUse == undefined) {
			    ...
			    // in the event a JAR/EAR file is being supplied as the model, need the Java Root Package Names to considre
			     input.javaRootPackageNames = appParams.model.javaRootPackageNames;
			...
			//console.log( "Input app options discovered as " + JSON.stringify(input ) );
			...
			if ( genYamlAsJson != null ) {
	    		...
	    		if ( Number.isInteger(modelIdentifierToUse) == false ) {
	    			console.log('Model identifer ' + modelIdentifierToUse + ' determined to be a file.  Validating/publishing model with server...');
	   				modelHandler.register(modelIdentifierToUse.toString(), null, null, appParams.model.javaRootPackageNames)
