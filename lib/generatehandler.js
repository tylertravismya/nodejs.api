...
let self = module.exports = {
	    			...
	    			if ( Number.isInteger(modelIdentifierToUse) == false ) {
	    				...
	    				modelHandler.register(modelIdentifierToUse.toString(), null)
	    					.then(function(result) {
	    						...
	    						console.log( 'Registered model identifier is ' + modelIdentifierToUse );
	    						self.internalGitHelper(appParams, techStackId, modelIdentifierToUse);
	    						
	    					}).catch(err => reject(err));
	    			}
	    			else {
	    				self.internalGitHelper(appParams, techStackId, modelIdentifierToUse, gitFile, appOptionsFile)
	    				.then(function(result) {
	    					console.log(result);
	    				}).catch(err => reject(err));
	    			}
	...
	internalGitHelper: ( appParams, techStackId, modelId, gitFile, appOptionsFile ) => {
		...
		return new Promise(async function(resolve, reject) {
			...
			console.log( 'git file input is ' + gitFile );
			...
			console.log( 'Git file is ' + gitFileToUse );
			
			fileHandler.loadYMLToJSON(gitFileToUse, function( err, data ){
				if ( err ) {
					reject( status.error( err, "Error parsing git yaml file " + gitFileToUse + " to json" ));
				}
				gitYamlAsJson = data;
			});
