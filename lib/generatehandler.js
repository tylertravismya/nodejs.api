...
let self = module.exports = {
	...
	generateApp: (yamlFile, gitFile, appOptionsFile) => {
			...
			if ( genYamlAsJson != null ) {
				...
				for(let index = 0; index < genYamlAsJson.app.length; index++ ) {
	    			...
	    			if ( Number.isInteger(modelId) == false ) {
	    				...
	    				self.internalGitHelper(appParams, techStackId, modelId, gitFile, appOptionsFile)
	    				.then(function(result) {
	    					console.log(result);
	    				}).catch(err => reject(err));
	    			}
	...
	internalGitHelper: ( appParams, techStackId, modelId, gitFile, appOptionsFile ) => {
		...
		return new Promise(async function(resolve, reject) {
			...
			if ( gitFileToUse == null ) // if one is not provided, use the one designated in the appParams file
			...
			var appOptionsFileToUe = appOptionsFile;

			if ( appOptionsFileToUe == null ) // if one is not provided, use the one designated in the appParams file
				appOptionsFileToUe = appParams.appOptionsFile,
