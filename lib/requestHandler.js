...
module.exports =  {
	...
	handleRequest : (input, msg, callback, multipart) => {
		...
		// use PRODUCTION env to shift server from localhost (test) to goframework realMethods server
		...
		if ( multipart == true ) {
			...
			request(host + ':' + port + endPoint + '?input=' + msgBody, { json: true }, (err, resp, body) => {
				if ( !conf.get(constants.QUIET_MODE))
					status.stop();
